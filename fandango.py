...
def parse(location, showdate):
	...
	location_url = 'https://www.fandango.com/napi/home/autocompleteDesktopSearch?search=' + searchedLocation
	data = {
		'zipCode': '',
		'city': '',
		'state': '',
	...
	print(location_url)
	raw_response = requests.get(location_url, cookies=cookie, headers=location_headers)
		...
		if searchdate >= datenow:
			...
			if scraped_data:
				...
				with open('%s-%s-movie-results.csv' % (location, showdate), 'wb')as csvfile:
					fieldnames = ['Theatre_Name', 'Theatre_Address', 'Movie_Name', 'Show_Date', 'Location_or_Zipcode', 'Duration', 'Genre', 'Movie_Rating', 'Star_Rating']
